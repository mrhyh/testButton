...
@property (nonatomic, copy) MGStringResultBlock resultBlock;
...
- (void)setManager:(MGPickerViewManager *)manager {
    _manager = manager;
}
...
- (instancetype)initWithFrame:(CGRect)frame dataSource:(NSArray *)dataSource
    ...
    if (_leftUnitLabel == nil) {
        _leftUnitLabel = [[UILabel alloc] initWithFrame:CGRectMake(_pickerView.frame.size.width / 4 + 10, _pickerView.frame.size.height/2 - 15, 50, 30)];
        _leftUnitLabel.text = @"小时";
        _leftUnitLabel.textAlignment = kCTTextAlignmentLeft;
        _leftUnitLabel.font = [UIFont systemFontOfSize:13];
        [_pickerView addSubview:_leftUnitLabel];
    }
    if (_rightUnitLabel == nil) {
        _rightUnitLabel = [[UILabel alloc] initWithFrame:CGRectMake(_pickerView.frame.size.width/4 * 3 + 20 , _leftUnitLabel.frame.origin.y, _leftUnitLabel.frame.size.width, _leftUnitLabel.frame.size.height)];
        _rightUnitLabel.text = @"分钟";
        _rightUnitLabel.textAlignment = kCTTextAlignmentLeft;
...
- (void)pickerView:(UIPickerView *)pickerView didSelectRow:(NSInteger)row inComponent:(NSInteger)component {
    ...
    if ([leftStr isEqualToString:@"0"] && [rightStr isEqualToString:@"0"]) {
...
@interface MGPickerViewManager ()

@end
@implementation MGPickerViewManager
...
- (void)encodeWithCoder:(NSCoder *)enCoder {
    [enCoder encodeInteger:self.defaultRow forKey:@"defaultRow"];
    [enCoder encodeInteger:self.defaultComponent forKey:@"defaultComponent"];
}

- (id)initWithCoder:(NSCoder *)decoder {
    if (self = [super init]) {
        self.defaultRow = [decoder decodeIntegerForKey:@"defaultRow"];
        self.defaultComponent = [decoder decodeIntegerForKey:@"defaultComponent"];
    }
    return self;
}
